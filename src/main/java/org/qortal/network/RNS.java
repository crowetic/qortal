...
//import org.qortal.network.task.ReticulumBroadcastTask;
//import org.qortal.network.task.ReticulumPrunePeersTask;
...
public class RNS {
        ...
        public void receivedAnnounce(byte[] destinationHash, Identity announcedIdentity, byte[] appData) {
            ...
            if (activePeerCount < MAX_PEERS) {
                ...
                if (!peerExists) {
                    ...
                    log.info("added new ReticulumPeer, destinationHash: {}", encodeHexString(destinationHash));
                }
            }
    ...
    //        Task task;
    //
    //        //// TODO: Needed? Figure out how to add pending messages in ReticulumPeer
    //        ////        (ReticulumPeer: pendingMessages.offer(message))
    ...
    //    ////            .map(ReticulumPeer::getMessageTask)
    //    ////            .filter(ReticulumPeer::isAvailable)
    //    ////            .findFirst()
    //    ////            .orElse(null);
    //    ////}
    //    //// Note: we might not need this. All messages handled asynchronously in Reticulum
    //    ////       (ReticulumPeer peerBufferReady callback)
    //    //private Task maybeProducePeerMessageTask() {
    ...
    //        return new ReticulumBroadcastTask();
    //    }
    //
    //    private Task maybeProduceReticulumPrunePeersTask(Long now) {
    //        if (now == null || now < nextPruneTimestamp.get()) {
    //            return null;
    //        }
    //        
    //        nextPruneTimestamp.set(now + PRUNE_INTERVAL);
    //        return new ReticulumPrunePeersTask();
    //    }
    //}
